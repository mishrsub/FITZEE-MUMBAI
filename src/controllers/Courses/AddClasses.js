...
class AddClass {
    addClass = async (req, res) => {
        try {
            ...
            if (!group) {
                return res
                    .status(404)
                    .json({ status: 404, message: "Group not found." });
            }

            // Create a new class associated with the found group
            const newClass = await ClassModel.findOneAndUpdate(
                { name: className, type: "programs" },
                { name: className, groupId: group._id },
    ...
    getClassAndProgramById = async (req, res) => {
        try {
            ...
            const pipeline = [
                ...
                {
                    $match: {
                        "programData._id": new mongoose.Types.ObjectId(
                            programId
                        ),
                    },
                },
    ...
    getAllClass = async (req, res) => {
        try {
            ...
            return res
                .status(200)
                .json({ status: 200, class: result, count: countClass });
        } catch (error) {
            return res.status(400).json({ status: 400, error: error.message });
        }
    };
            ...
            const getProgram = await ProgramModel.aggregate([
                {
                    $match: {
                        ...
                        "subprograms._id": new mongoose.Types.ObjectId(
                            subProgramId
                        ),
                    },
                },
    ...
    editProgramDetail = async (req, res) => {
        try {
            ...
            return res
                .status(200)
                .json({ status: 200, programDetail: editDetail });
        } catch (error) {
            return res.status(400).json({ status: 400, error: error.message });
        }
    };
        ...
        try {
            ...
            const averageRating =
                subProgram.numOfReviews === 0
                    ? rating
                    : parseFloat(
                          (totalRating + rating) / (subProgram.numOfReviews + 1)
                      );
    ...
    getReviews = async (req, res) => {
        try {
            ...
            return res
                .status(200)
                .json({ status: 200, reviews: reviews.review });
        } catch (error) {
            return res.status(400).json({ status: 400, error: error.message });
        }
    };
    ...
    addGroup = async (req, res) => {
        try {
            ...
            return res
                .status(200)
                .json({ status: 200, message: "Group added successfully." });
        } catch (error) {
            return res.status(400).json({ status: 400, error: error.message });
        }
    };
        ...
        try {
            ...
            const groups = await GroupModel.aggregate([
                ...
                },
                {
                    $lookup: {
                        from: "programs",
                        localField: "classData.programs",
                        foreignField: "_id",
                        as: "classData.programData",
                    },
                },
                // {
                //     $lookup: {
                ...
                // },
                {
                    $group: {
                        _id: "$_id",
                        name: { $first: "$name" },
                        classData: { $push: "$classData" }, // Push back the classData array
                    },
                },
                {
                    $project: {
            ...
            return res.status(200).json({ status: 200, groups: groups[0] });
        } catch (error) {
            return res.status(400).json({ status: 400, error: error.message });
        }
    };
...
// After the initial $lookup stage, we use $unwind to deconstruct the classData array.
// Then, we perform another $lookup stage to join the programs collection based on the programs array within classData.
// After that, we use $group to regroup the data by _id and push the classData back into an array.
// Finally, we use $project to shape the output document as desired.
